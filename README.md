...
~~~
sound play 3 loop 5 sound play "Siren1.wav" sound vol 160
sound treble -6
sound fx 9 0x04 0 0
~~~
...
`channels` can be specified as a single channel number 1 or 2 (or as a and b), a list of channels
enclosed with [] parenthesis, or the keyword all
...
motor all stop
motor a speed -50 motor 2 servo 45 motor 1 fx 0x07 0x03 0
...
`run fileID` - execute script with fileID `stop` - stops the script at the current line
...
#
# Ch 1: Red, Ch 2: Yellow, Ch 3: Green # Ch 4: Don't Walk, Ch 5: Walk
...
wait 4
# Start the sequence again repeat
```
